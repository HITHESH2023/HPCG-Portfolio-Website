  ...
  return (
    <group>
      <group position={[-3, 1.8, -1]}>
        <BillboardBadge label="React" />
      </group>
      <group position={[-2.3, -1.5, -1]}>
        <BillboardBadge label="Node.js" />
      </group>
      <group position={[2.8, 0.9, -1]}>
        <BillboardBadge label="MongoDB" />
      </group>
      <group position={[3.2, -1.2, -1]}>
        <BillboardBadge label="PostgreSQL" />
      </group>
      <group position={[1.2, 2.4, -1]}>
        <BillboardBadge label="Java" />
      </group>
    </group>
  );
}
...
// Project preview with small 3D object
function ProjectPreview({ seed = 1 }) {
  // simple animated icosahedron
  const ref = useRef();
  useFrame((state, delta) => {
    if (!ref.current) return;
    ref.current.rotation.x += delta * 0.3 * (1 + seed * 0.05);
    ref.current.rotation.y += delta * 0.2 * (1 + seed * 0.03);
  });
  return (
    <mesh ref={ref}>
      <icosahedronGeometry args={[1.0 + seed * 0.06, 0]} />
          ...
          {CONFIG.projects.map((p, i) => (
            ...
            >
              <Card className="h-full">
                <div className="h-36 rounded-xl overflow-hidden border border-white/10 bg-gradient-to-br from-indigo-500/30 to-cyan-500/30 grid place-items-center">
                  <Canvas camera={{ position: [0, 0, 4.2], fov: 50 }}>
                    <ambientLight intensity={0.7} />
                    <directionalLight position={[5, 5, 5]} intensity={0.9} />
                    <Suspense fallback={null}>
                      <ProjectPreview seed={i + 1} />
                    </Suspense>
